...
uint8_t CHECK_COLLIDER(void){
    ...
    if(THIS_ACTOR->x < THIS_COLLIDER->x + adjusted_width &&
       THIS_ACTOR->x + get_half_width() > THIS_COLLIDER->x &&
       THIS_ACTOR->y < THIS_COLLIDER->y + adjusted_height &&
       THIS_ACTOR->y + get_half_height() > THIS_COLLIDER->y) {
        return 1;
    }
    return 0;
}
...
uint16_t GET_RIGHT(void){
    return THIS_COLLIDER->x + ((BoxCollider*)THIS_COLLIDER)->width;
}

uint16_t GET_BOTTOM(void){
    return THIS_COLLIDER->y + ((BoxCollider*)THIS_COLLIDER)->height;
}